...
async def build(project_id: uuid.UUID, org_id: uuid.UUID, db: AsyncSession,
    ...
    # No threshold special-case here any more. It fired only at ~100% Direct,
    # so a single genuine referral dropped the rest below the cutoff while the
    # ambiguity was identical. classify_referrer now names the bucket
    # "Direct or unattributed", which is honest at every share.
