...
async def run_market_monitor(ctx) -> None:
    """Snapshot-diff monitoring (see monitoring_service) is deterministic --
    zero LLM calls, so this never enters batch_scope()."""
    ...
    for p in projects:
        ...
        except Exception:  # noqa: BLE001 - one project must not break the run
            logger.exception("market monitor failed for project %s", p.id)


async def run_competitor_monitor(ctx) -> None:
    """Scorecard-diff monitoring (see monitoring_service/competitor_service) is
    deterministic HTTP scraping -- zero LLM calls, so this never enters
    batch_scope()."""
    async with async_session_factory() as db:
        projects = (await db.execute(
